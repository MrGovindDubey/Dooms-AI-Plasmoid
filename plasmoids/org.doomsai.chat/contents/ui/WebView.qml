    ...
    WebEngineView {
        ...
        onLoadingChanged: function(loadRequest) {
            if (loadRequest.status === WebEngineView.LoadSucceededStatus) {
                ...
                `)
                // Replay current status into HTML now that it is fully loaded
                if (webViewRoot.rootItem) {
                    webViewRoot.rootItem.updateHtmlStatus(webViewRoot.rootItem.statusState, webViewRoot.rootItem.statusText)
                }
